export const voices = [
  { language: 'FR', languageCode: 'fr-FR', name: 'Français', value: 'fr-FR-Standard-D', genre: 'Male' },
  { language: 'FR', languageCode: 'fr-FR', name: 'Français', value: 'fr-FR-Wavenet-C', genre: 'Female' },
  { language: 'FR', languageCode: 'fr-CA', name: 'Québécois', value: 'fr-CA-Wavenet-B', genre: 'Male' },
  { language: 'FR', languageCode: 'fr-CA', name: 'Québécois', value: 'fr-CA-Neural2-A', genre: 'Female' },
  { language: 'EN', languageCode: 'en-US', name: 'Anglais', value: 'en-US-Standard-D', genre: 'Male' },
  { language: 'DE', languageCode: 'de-DE', name: 'Allemand', value: 'de-DE-Polyglot-1', genre: 'Male' },
  { language: 'CH', languageCode: 'yue-HK', name: 'Chinois', value: 'yue-HK-Standard-D', genre: 'Male' },
];

export const versions = [
  { name: 'GPT-4 Turbo', value: 'gpt-4-0125-preview' },
  { name: 'GPT-4', value: 'gpt-4' },
  { name: 'GPT 3.5 Turbo', value: 'gpt-3.5-turbo-0125' },
];
